...
`with-display`."
  (if (not name)
	  (wayvment.ffi-client::wl-display-connect (or +envar-wl-display-xdg+
												   +envar-wl-display-xdg-wl-0+))
	  (wayvment.ffi-client::wl-display-connect name)))
...
In most cases, use `with-display`."
  (wayvment.ffi-client::wl-display-disconnect display))
...
`wl-display-roundtrip returns -1 on failure, so we set it accordingly."
  (let ((res (wayvment.ffi-client:wl-display-roundtrip display)))
	(if (minusp res)
		nil
		t)))
