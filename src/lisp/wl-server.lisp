...
(defun display-destroy (display)
  (wayvment.ffi-server:wl-display-destroy display)
  (setf display nil))
...
Unless not possible, use `with-display`."
  (wayvment.ffi-server:wl-display-create))

(defmacro with-display (display &body body)
  `(let ((,display (wayvment.ffi-server:wl-display-create)))
...
the socket from, or to auto-add a socket."
  (cond ((integerp sock)
		 (let ((val (wayvment.ffi-server:wl-display-add-socket-fd display sock)))
		   (if (minusp val)
			   (format t "Failed to bind to socket!")
			   val)))
		((stringp sock)
		 (let ((val (wayvment.ffi-server:wl-display-add-socket display sock)))
		   (if (minusp val)
			   (format t "Failed to create socket!")
			   val)))
		(t
		 (let ((val (wayvment.ffi-server:wl-display-add-socket-auto display)))
		   (if (not val)
			   (format t "Failed to create socket!")
			   val)))))
